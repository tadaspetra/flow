...
/** Shape of a single token as received from the Scribe transcription engine. */
...
/** A spoken-word token with the text guaranteed to be a non-empty string. */
 ...
 */
export function extractSpokenWordTokens(tokens: unknown[]): SpokenWordToken[] {
  const spoken: SpokenWordToken[] = [];
  let parenDepth = 0;
  let bracketDepth = 0;

  for (const token of Array.isArray(tokens) ? tokens : []) {
    const scribeToken = token as ScribeToken | null | undefined;
    const rawText = typeof scribeToken?.text === 'string' ? scribeToken.text : '';
    ...
    if (scribeToken?.type === 'word' && !isInsideAnnotation) {
      spoken.push({
        text: scribeToken.text as string,
        ...(scribeToken.start !== undefined ? { start: scribeToken.start } : {}),
        ...(scribeToken.end !== undefined ? { end: scribeToken.end } : {})
      });
    }
