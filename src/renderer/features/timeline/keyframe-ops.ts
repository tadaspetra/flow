...
import type { Keyframe } from '../../../shared/types/domain.js';

/** Minimal defaults bag for building a split anchor keyframe. */
interface SplitAnchorDefaults {
  pipX: number;
  pipY: number;
 ...
 */
export function reindexSections(sections: Array<{ index: number; label: string }>): void {
...
export function buildSplitAnchorKeyframe(
  ...
  defaults: SplitAnchorDefaults
): Omit<Keyframe, 'pipSnapPoint' | 'autoTrack' | 'autoTrackSmoothing' | 'backgroundFocusX' | 'backgroundFocusY'> {
