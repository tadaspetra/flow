...
export function resolveOutputSize(
  ...
  if (outputMode === 'reel') {
    const outH = sourceHeight % 2 === 0 ? sourceHeight : sourceHeight - 1;
...
interface ScaledKeyframe extends Keyframe {
  pipScale: number;
}
...
export function buildNumericExpr(
  ...
  // Helper to access keyframe property dynamically (Keyframe has no index signature)
  const getField = (kf: Keyframe): unknown => kf[prop];

  const firstKf = keyframes[0];
  const firstRaw = firstKf ? getField(firstKf) : undefined;
  const firstValue = Number.isFinite(Number(firstRaw))
    ? Number(firstRaw)
    : defaultValue;
  ...
  for (let i = 1; i < keyframes.length; i += 1) {
    ...
    const prevRaw = getField(prev);
    const currRaw = getField(curr);
    const prevVal = Number.isFinite(Number(prevRaw)) ? Number(prevRaw) : defaultValue;
    const currVal = Number.isFinite(Number(currRaw)) ? Number(currRaw) : defaultValue;
    const t = curr.time;
...
export function panToFocusCoord(zoom: unknown, pan: unknown, defaultCoord: number = 0.5): number {
...
export function buildCamFullAlphaExpr(keyframes: Keyframe[]): string {
  const isFullVisible = (keyframe: Keyframe): boolean => (keyframe.cameraFullscreen || false) && keyframe.pipVisible;
...
export function buildScreenFilter(
  ...
  canvasW: number,
  _canvasH: number,
  outputLabel: string = '[screen]',
  screenPreprocessed: boolean = false,
  targetFps: number = 30,
  outputMode: OutputMode = 'landscape'
): string {
  ...
  const normalizedKeyframes: NormalizedKeyframe[] = (Array.isArray(keyframes) ? keyframes : []).map((keyframe) => ({
    ...keyframe,
...
export function buildFilterComplex(
  ...
  const normalizedKeyframes: ScaledKeyframe[] = (Array.isArray(keyframes) ? keyframes : []).map((kf) => ({
    ...kf,
  ...
  function buildCamPipFilter(inputLabel: string, outputLabel: string): string {
 ...
 * Returns { inputs, filterParts, outputLabel } where:
