...
export const COMMON_FRAME_RATES = [24, 25, 30, 50, 60] as const;
...
export function parseVideoFpsFromProbeOutput(output: string): number | null {
  ...
  for (const pattern of patterns) {
    ...
    if (Number.isFinite(fps) && fps !== null && fps > 0) return fps;
  }

  return null;
}

export function probeVideoFpsWithFfmpeg(ffmpegPath: string, filePath: string): Promise<number | null> {
...
export function chooseRenderFps(candidates: (number | null)[], hasCamera: boolean): number {
