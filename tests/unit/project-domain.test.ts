...
describe('shared/domain/project', () => {
  ...
  test('normalizeKeyframes preserves reel zoom-out values (0.5-1.0)', () => {
    ...
    expect(keyframes[0]!.backgroundZoom).toBe(0.7);
    expect(keyframes[1]!.backgroundZoom).toBe(0.5);
    expect(keyframes[2]!.backgroundZoom).toBe(0.5);
  });
  ...
  test('normalizeKeyframes includes pipSnapPoint property', () => {
    ...
    expect(keyframes[2]!.pipSnapPoint).toBe('br');
    expect(keyframes[3]!.pipSnapPoint).toBe('br');
  });
  ...
  test('normalizeKeyframes includes autoTrack and autoTrackSmoothing', () => {
    ...
    expect(keyframes[1]!.autoTrack).toBe(false);
    expect(keyframes[1]!.autoTrackSmoothing).toBe(0.15);
    expect(keyframes[2]!.autoTrackSmoothing).toBe(0.01);
    expect(keyframes[3]!.autoTrackSmoothing).toBe(1.0);
  });
