import { deriveProxyPath } from '../../src/main/services/proxy-service.js';
import type { GenerateProxyOptions } from '../../src/main/services/proxy-service.js';

// Each test needs a fresh module because the concurrency queue is module-level state.
// We re-import via vi.resetModules() + dynamic import() to reset the queue between tests.
async function freshGenerateProxy(): Promise<
  (opts: GenerateProxyOptions, deps?: Record<string, unknown>) => Promise<void>
> {
  vi.resetModules();
  const mod = await import('../../src/main/services/proxy-service.js');
  return mod.generateProxy as (opts: GenerateProxyOptions, deps?: Record<string, unknown>) => Promise<void>;
}

function makeFakeFs({ existsSync = () => false }: { existsSync?: (p: string) => boolean } = {}) {
  return {
    existsSync,
    unlinkSync: vi.fn(),
    renameSync: vi.fn()
...
function makeRunFfmpeg(exitCode = 0, delay = 0) {
  return vi.fn(() => new Promise<{ stderr: string }>((resolve, reject) => {
    const settle = () => {
...
describe('proxy-service -- generateProxy', () => {
  test('happy path: ffmpeg exits 0 -> renames .tmp to final path', async () => {
    ...
    const callArgs = fakeRun.mock.calls[0]![0] as { args: string[] };
  ...
  test('concurrency: 3 calls with max 2 -> only 2 run at once, 3rd starts after first completes', async () => {
    ...
    const fakeRun = vi.fn(() => new Promise<{ stderr: string }>((resolve) => {
      active.count += 1;
    ...
    const opts = (n: number) => ({ screenPath: `/proj/s${n}.webm`, proxyPath: `/proj/s${n}-proxy.mp4`, ffmpegPath: '/ffmpeg' });
